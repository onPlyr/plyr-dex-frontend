...
export default function Header({ setIsReset, requestError }: { setIsReset: (isReset: boolean) => void, requestError: string[] }) {
        ...
        <div className={`${styles.header} aboslute text-white w-full top-0  h-16 flex flex-row px-6 items-center justify-between`}>
            <div className="flex flex-row items-center justify-center gap-2">
                ...
                <div className="flex flex-col items-start justify-center">
                    ...
                    <div className="text-[13px] font-light leading-none" style={{ lineHeight: '12px' }}>CONNECT</div>
                </div>
            </div>
            {
              requestError.length === 0 && <a onClick={() => setIsReset(true)} className="cursor-pointer text-sm font-bold text-white">
                    <Settings className="w-6 h-6" style={{ strokeWidth: 1.5 }} />
                </a>
            }
        </div>
    </section>
