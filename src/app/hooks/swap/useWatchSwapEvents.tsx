...
export type EventQuery = HopEventQuery | DstTxEventQuery
export type EventQueryMap = Map<number, EventQuery[]>
export type EventQueryResult = HopEventQueryResult | DstTxEventQueryResult
...
export const isHopEventQueryResult = (data: EventQueryResult): data is HopEventQueryResult => data.hopIndex !== undefined && !data.isDstTxQuery
...
export const getSwapEventQuery = (swap: SwapHistory): EventQuery | undefined => {
    ...
    const pendingHop = swap.hops.find((hop) => hop.status === SwapStatus.Pending)
    const msgId = pendingHop ? (pendingHop.msgReceivedId ?? swap.hops.find((hop) => hop.index === pendingHop.index - 1)?.msgSentId) : swap.hops.at(-1)?.msgSentId
    return msgId && {
        chainId: pendingHop?.srcData.chain.id ?? swap.dstData.chain.id,
        swapTxHash: swap.txHash,
        msgId: msgId,
        hopIndex: pendingHop?.index,
        isDstTxQuery: !pendingHop,
        fromBlock: pendingHop ? pendingHop.lastCheckedBlock || pendingHop.initiatedBlock : swap.dstLastCheckedBlock || swap.dstInitiatedBlock,
    }
}

export const getSwapEventQueryMap = (swapHistories: SwapHistory[]): EventQueryMap => {
    const queries = swapHistories.filter((swap) => swap.status === SwapStatus.Pending).map((swap) => getSwapEventQuery(swap)).filter((data) => !!data)
